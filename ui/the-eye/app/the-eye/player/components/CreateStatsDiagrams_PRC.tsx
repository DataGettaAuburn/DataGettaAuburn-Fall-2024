...
{    
    ...
    const batter = await prisma.$queryRaw<batter_stats[]>`SELECT * FROM get_batter_stats_prc(${player}, ${team}, ${startDate}::date, ${endDate}::date)`;
    const pitcher = await prisma.$queryRaw<pitcher_stats[]>`SELECT * FROM get_pitcher_stats_prc(${player}, ${team}, ${startDate}::date, ${endDate}::date)`;
    const pitches = await prisma.$queryRaw<pitch_sums[]>`SELECT * FROM get_pitch_count_prc(${player}, ${team}, ${startDate}::date, ${endDate}::date)`;
